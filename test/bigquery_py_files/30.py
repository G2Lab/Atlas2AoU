...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (43012518,1836191,1782521,1763204,1749301,1759455,1777417,19035953,1710446,1750074,1726228,19078424,19135812,19010337,19026710,1742253,1716903,1736887,1798476,36879076,1797258,1778262,1709170,1790868)
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (45771090,4112923,4090539,258675,432541,434557)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
select c.person_id, c.drug_era_id as event_id, c.drug_era_start_date as start_date, c.drug_era_end_date as end_date,
    cast(null  as int64) as visit_occurrence_id,c.drug_era_start_date as sort_date, c.drug_concept_id as domain_concept_id
from 
(
  select de.* 
  from {os.environ["WORKSPACE_CDR"]}.drug_era de
