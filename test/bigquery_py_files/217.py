...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4006469,37397537)
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (317510,432571,437233,3654297,3654352,4005251,4030264,4043444,4043445,4043713,4058706,4156433,4161020,4161138,4161560,4161665,4164159,4221907,4224628,4241295,4295651,4301659,4301780,4323972,4347562,36680584,37016161,37204530,37206728,42538056,45767656)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (2212705,1314273,4182536,4049372,4022139)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
SELECT
event_id, person_id, 
  case when DATE_ADD(IF(SAFE_CAST(end_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(end_date  AS STRING)),SAFE_CAST(end_date  AS DATE)), INTERVAL 7 DAY) > op_end_date then op_end_date else DATE_ADD(IF(SAFE_CAST(end_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(end_date  AS STRING)),SAFE_CAST(end_date  AS DATE)), INTERVAL 7 DAY) end as end_date
FROM
included_events;
