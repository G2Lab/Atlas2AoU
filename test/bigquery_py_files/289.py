...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (21600248)
...
left join
(
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (989878)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (45890325,45890400,2107068,4051932,4051938,2313791,2756785,4019139,2615874,2615956,2615795,2615958,2615957,2615796,2100778,2313929)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (44784217,38001137,38001138,4169095,4103295,4171903,441139,442289,435937)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
select c.person_id, c.device_exposure_id as event_id, c.device_exposure_start_date as start_date, coalesce(c.device_exposure_end_date, DATE_ADD(IF(SAFE_CAST(c.device_exposure_start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(c.device_exposure_start_date  AS STRING)),SAFE_CAST(c.device_exposure_start_date  AS DATE)), INTERVAL 1 DAY)) as end_date,
        c.visit_occurrence_id, c.device_exposure_start_date as sort_date
from 
(
  select de.* 
  from {os.environ["WORKSPACE_CDR"]}.device_exposure de
