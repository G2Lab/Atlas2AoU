...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4091559)
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4048018,36716396)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (44782781,43530623,381316)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) a on a.person_id = p.person_id  and a.start_date >= DATE_ADD(IF(SAFE_CAST(p.start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(p.start_date  AS STRING)),SAFE_CAST(p.start_date  AS DATE)), INTERVAL -183 DAY) and a.start_date <= DATE_ADD(IF(SAFE_CAST(p.start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(p.start_date  AS STRING)),SAFE_CAST(p.start_date  AS DATE)), INTERVAL -1 DAY) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
