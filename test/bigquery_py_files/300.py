...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4302555,45757810)
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (443213,437611,4067106,433260)
) i
) c union all 
select 3 as codeset_id, c.concept_id from (select distinct i.concept_id from
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4010333,4141640,42536667,4172857,37110086,4059477,4144036)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (44827910,35209473)
) i
) c
;
...
select c.person_id, c.condition_era_id as event_id, c.condition_era_start_date as start_date,
       c.condition_era_end_date as end_date, cast(null  as int64) as visit_occurrence_id,
       c.condition_era_start_date as sort_date
from 
(
  select ce.* 
  from {os.environ["WORKSPACE_CDR"]}.condition_era ce
...
) a on a.person_id = p.person_id  and a.start_date <= DATE_ADD(IF(SAFE_CAST(p.start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(p.start_date  AS STRING)),SAFE_CAST(p.start_date  AS DATE)), INTERVAL 0 DAY) and a.end_date >= DATE_ADD(IF(SAFE_CAST(p.start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(p.start_date  AS STRING)),SAFE_CAST(p.start_date  AS DATE)), INTERVAL 0 DAY) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
