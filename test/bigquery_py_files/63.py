...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (443904)
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (437113,79908,138965,4236484,135526)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) a on a.person_id = p.person_id  and a.start_date >= DATE_ADD(IF(SAFE_CAST(p.start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(p.start_date  AS STRING)),SAFE_CAST(p.start_date  AS DATE)), INTERVAL -365 DAY) and a.start_date <= DATE_ADD(IF(SAFE_CAST(p.start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(p.start_date  AS STRING)),SAFE_CAST(p.start_date  AS DATE)), INTERVAL -1 DAY) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
...
SELECT
event_id, person_id, 
  case when DATE_ADD(IF(SAFE_CAST(start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(start_date  AS STRING)),SAFE_CAST(start_date  AS DATE)), INTERVAL 1 DAY) > op_end_date then op_end_date else DATE_ADD(IF(SAFE_CAST(start_date  AS DATE) IS NULL,PARSE_DATE('%Y%m%d', cast(start_date  AS STRING)),SAFE_CAST(start_date  AS DATE)), INTERVAL 1 DAY) end as end_date
FROM
included_events;
