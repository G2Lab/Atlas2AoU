...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4051938,2313791,2756785,4019139,2615874,2615956,2615795,2615958,2615957,2615796)
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (320425,317302,4305210,4169095)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
