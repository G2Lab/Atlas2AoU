...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (443392,4144289)
...
left join
(
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4112752,4111921,4179242)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
