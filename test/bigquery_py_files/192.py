...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (38001002,4051478,45768610,2106308,4262407,4055146,4049224,4181851,4082142,4182603,4059014,4096682,4118793,4177600,38001003,45772698,4311406,40490890,4309836,4300859)
...
left join
(
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4307580,4067764,4080556,4311252,4298842,4300127,4300126,4300128,4071068,4006156,4012540,4209297,4299540,4100838,4100839,4154597,4294426,4119134,4216866,4096219,4189651,4033871,4123077)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
