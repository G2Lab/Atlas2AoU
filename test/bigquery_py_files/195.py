...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (197508,4177230,192855)
...
left join
(
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (4312149,4312802,4313210,4314336,4313925,4314491,4313221,4313222,4314494,37016119,4113112,4111028,37116429,36713671,4080733,4175529,4315813)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
