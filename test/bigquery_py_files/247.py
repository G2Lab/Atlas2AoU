...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (762049,44782752,435565,40481089,193512,4133004,4149782,44782762,762048,46271900,4317289,4124856,4318407,4055089,45757410,45757409,4203836,44782759,4205652,762046,762051,44811347,4033521,45773536,4203618,762047,36712892,4096099)
...
left join
(
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (36712971,435887,4179911,435031,4062264,438820,4089917,4269051)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
