...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (377889,40484010,434046,379831,433496,40659147)
...
left join
(
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (376793)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
