...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (437854,436100)
...
( 
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (438134)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
...
left join
(
  select concept_id from {os.environ["WORKSPACE_CDR"]}.concept where concept_id in (436100,437854)
union distinct select c.concept_id
  from {os.environ["WORKSPACE_CDR"]}.concept c
  join {os.environ["WORKSPACE_CDR"]}.concept_ancestor ca on c.concept_id = ca.descendant_concept_id
