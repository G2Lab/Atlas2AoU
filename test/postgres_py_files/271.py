...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4144746,28396,4009785,4122936,4301410,444238)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (4149681,25518,4121113,4098749,4125628,4296149,4252396,4262394,4121114,40484201)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE <= (P.START_DATE + 60*INTERVAL'1 day') ) cc 
GROUP BY cc.person_id, cc.event_id
