...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (380378,377091)
...
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (444413,44783583,377101)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4236312,380378,43531630,4029498)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (45757050,444413,44783583,46270370,377101)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (790253,713192)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (40483317,4309257)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
JOIN ukbb.VISIT_OCCURRENCE V on C.visit_occurrence_id = V.visit_occurrence_id and C.person_id = V.person_id
WHERE C.condition_start_date >= TO_DATE(TO_CHAR(2010,'0000')||'-'||TO_CHAR(1,'00')||'-'||TO_CHAR(1,'00'), 'YYYY-MM-DD')
...
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + -1825*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + -30*INTERVAL'1 day') ) cc 
...
WHERE C.drug_exposure_start_date >= TO_DATE(TO_CHAR(2010,'0000')||'-'||TO_CHAR(1,'00')||'-'||TO_CHAR(1,'00'), 'YYYY-MM-DD')
AND C.ordinal = 1
...
WHERE C.ordinal = 1
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + -730*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + 0*INTERVAL'1 day') ) cc 
GROUP BY cc.person_id, cc.event_id
HAVING COUNT(cc.event_id) >= 1
-- End Correlated Criteria
  ) CQ on E.person_id = CQ.person_id and E.event_id = CQ.event_id
