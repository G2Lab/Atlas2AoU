...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (21600248)
...
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (989878)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (45890325,45890400,2107068,4051932,4051938,2313791,2756785,4019139,2615874,2615956,2615795,2615958,2615957,2615796,2100778,2313929)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (44784217,38001137,38001138,4169095,4103295,4171903,441139,442289,435937)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
select C.person_id, C.device_exposure_id as event_id, C.device_exposure_start_date as start_date, COALESCE(C.device_exposure_end_date, (C.device_exposure_start_date + 1*INTERVAL'1 day')) as end_date,
        C.visit_occurrence_id, C.device_exposure_start_date as sort_date
from 
(
  select de.* 
  FROM ukbb.DEVICE_EXPOSURE de
