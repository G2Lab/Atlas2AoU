...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4289908,314383,318072,44782774,4138837,4149913)
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4289908,318072,46273471,4299935,319720,4272028,4093494,312543)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + 0*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + 7*INTERVAL'1 day') ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
