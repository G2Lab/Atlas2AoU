...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (439847,432923,4148906,4144154,4111709,42535426,43530728,43530727,4108952,4111708,35609033)
) I
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (376713,4174299,36716544,4345688)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
) A on A.person_id = P.person_id  AND A.START_DATE >= (P.START_DATE + -180*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + -1*INTERVAL'1 day') ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
