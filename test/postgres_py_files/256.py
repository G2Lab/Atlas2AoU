...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4091559)
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4048018,36716396)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (44782781,43530623,381316)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= (P.START_DATE + -183*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + -1*INTERVAL'1 day') ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
...
WHERE C.ordinal = 1
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + -7*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + 7*INTERVAL'1 day') ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
  ) CQ on E.person_id = CQ.person_id and E.event_id = CQ.event_id
