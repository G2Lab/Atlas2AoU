...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (194385,46271022,192279,4263367,261071,201313,4103224,193253,195314,192359,45768812)
...
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (45769152,195289,195737,43530912,4066005,37116834,195014,197930,197320)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4090651,4032243,45889365,4027133,38003431)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + 30*INTERVAL'1 day') AND A.START_DATE <= P.OP_END_DATE ) cc 
