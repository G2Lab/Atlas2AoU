...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4006469,37397537)
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (317510,432571,437233,3654297,3654352,4005251,4030264,4043444,4043445,4043713,4058706,4156433,4161020,4161138,4161560,4161665,4164159,4221907,4224628,4241295,4295651,4301659,4301780,4323972,4347562,36680584,37016161,37204530,37206728,42538056,45767656)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (2212705,1314273,4182536,4049372,4022139)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + 0*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + 30*INTERVAL'1 day') ) cc 
...
SELECT
event_id, person_id, 
  case when (end_date + 7*INTERVAL'1 day') > op_end_date then op_end_date else (end_date + 7*INTERVAL'1 day') end as end_date
FROM
included_events;
