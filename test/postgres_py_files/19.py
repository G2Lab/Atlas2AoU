...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (4318404)
...
LEFT JOIN
(
  select concept_id from ukbb.CONCEPT where concept_id in (46269693,4193174,36712839,4273378,256622)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from ukbb.CONCEPT where concept_id in (36712839,4193174,256622,46269693,4273378)
UNION  select c.concept_id
  from ukbb.CONCEPT c
  join ukbb.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
-- End Condition Occurrence Criteria
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= (P.START_DATE + -1*INTERVAL'1 day') AND A.START_DATE <= (P.START_DATE + -1*INTERVAL'1 day') ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
